...
int nivelV (ABin a, int n, int v[]){
	...
	if(n==1){
		...
		ret += nivelV(a->esq, n-1, v);
		v = v+ret;
		ret += nivelV(a->dir, n-1, v);
	}
...
int dumpAbin (ABin a, int v[], int N){
	...
	ret += dumpAbin(a->esq, v, N-1);

	v[ret] = a->valor;
	ret++;
	N = N-ret;

	ret += dumpAbin(a->dir, v+ret, N);

	return ret;
}

//91
int somaAcABin(ABin a){
	int ret = 0;

	if(a){
		ret = a->valor + somaAcABin(a->esq) + somaAcABin(a->dir);
	}
...
int contaFolhas (ABin a){
	...
	if(a){
		ret = contaFolhas(a->esq) + contaFolhas(a->dir);
	}
...
int quantosMaiores (ABin a, int x){
	...
	if(a){
		if(a->valor > x) ret++;
		ret += quantosMaiores(a->esq, x);
		ret += quantosMaiores(a->dir, x);
	}
...
int menores (ABin a, int x){
	...
	ret = a->valor < x && menores(a->esq, x) && menores(a->dir, x);
...
int maiores (ABin a, int x){
	...
	ret = a->valor > x && maiores(a->esq, x) && maiores(a->dir, x);
...
int deProcura (ABin a){
	...
	ret = menores(a->esq, a->valor) && maiores(a->dir, a->valor) && deProcura(a->esq) && deProcura(a->dir);
